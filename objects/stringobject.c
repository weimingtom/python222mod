...
static PyObject *string_lower(PyStringObject *self)
{
	...
	new_ = PyString_FromStringAndSize(NULL, n);
	if (new_ == NULL)
	{
		return NULL;
	}
	s_new = PyString_AsString(new_);
	for (i = 0; i < n; i++) 
	{
		int c = Py_CHARMASK(*s++);
		if (isupper(c)) 
...
static PyObject *string_replace(PyStringObject *self, PyObject *args)
{
	...
	if (out_len == -1) 
	{
		...
		{
			new_ = PyString_FromStringAndSize(str, len);
			if (new_ == NULL)
			{
				return NULL;
			}
	...
	{
		new_ = PyString_FromStringAndSize(new_s, out_len);
		PyMem_FREE(new_s);
	}
	return new_;
}
